...
def fetch_yfinance_data(symbol, period='90d', interval='1h'):
    ...
    try:
        ...
        # Ensure proper column names (yf.download already uses Title Case)
        # Drop rows with any NaN values, which is a common issue with fetched data
        df = df[['Open', 'High', 'Low', 'Close', 'Volume']].dropna()
        
        if not df.empty:
            print(f"   ✅ Success with yfinance.download for {symbol}!")
            return df
        else:
